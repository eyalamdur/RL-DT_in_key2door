...
class KeyToDoorEnv(gym.Env):
    ...
    Rules:
      - Each room is a 4x4 grid (coords 0..3)
      - Agent always spawns in each room at (3,0)
      - Agent has 10 steps per room, then teleports to next room
      - Success only if agent reaches door (room 2, at (0,2)) *while holding key*
    ...
    def __init__(self, render_mode=None, max_room_steps=10):
        self.render_mode = render_mode
        self.max_room_steps = max_room_steps

        self.observation_space = spaces.Dict({
            "room": spaces.Discrete(3),
            "pos": spaces.Box(low=0, high=3, shape=(2,), dtype=np.int32),
        ...
        max_episode_steps = 3 * max_room_steps
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.room = 0
        self.pos = np.array([3,0], dtype=np.int32)
        self.has_key = 0
        self.room_steps = 0
    ...
    def step(self, action):
        ...
        if action == 0:      # up
            ...
            self.pos[0] = min(3, self.pos[0] + 1)
        elif action == 2:    # left
            self.pos[1] = max(0, self.pos[1] - 1)
        elif action == 3:    # right
            self.pos[1] = min(3, self.pos[1] + 1)
        elif action == 4:    # pick key
            if self.room == 0 and (self.pos[0] == 2 and self.pos[1] == 2):
                self.has_key = 1

        # Success check (only room 2)
        if self.room == 2 and self.has_key == 1 and (self.pos[0] == 0 and self.pos[1] == 2):
            reward = 10
            terminated = True
